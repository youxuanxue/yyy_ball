...
INPUT_FILE = "sunzi_bingfa_details.json"
OUTPUT_MD = "孙子兵法_大王与你读兵法_完整版.md"
...
def parse_post(post):
    ...
    if 'talk' in post:
        ...
        # Some long articles use this type
        content = post['article'].get('title', '') + "\n\n" + post['article'].get('text', '') # text might be empty for articles, usually inline html
        if not post['article'].get('text'):
             content += "(文章链接或富文本内容，此处仅展示标题)"
        images = post['article'].get('images', [])

    # If content is still empty, try generic 'text' field (seen in some APIs)
    if not content:
        content = post.get('text', '')

    # Comments (detail API usually has 'show_comments' or 'comments' list)
    comments = []
...
def main():
    ...
    # Filter out empty or None posts if any
