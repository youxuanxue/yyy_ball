...
仓库现已内置一个轻量 `creator` 工作台，用于承接上游 `yyy_monkey/creator` 中适合迁移的“内容抓取 -> 分析 -> 人工确认 -> 规划”能力，但不把它并成第二个独立 Python 项目。

- 代码入口：`src/creator/`
- 配置目录：`creator/config/target.json`
- 输出目录：`creator/output/`
- 使用方式：
...
设计审计与迁移理由见：`creator/ARCHITECTURE_REVIEW.md`。
