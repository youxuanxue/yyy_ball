...
系列专属的维护脚本也应尽量随 skill 收口，而不是继续堆在 `src/` 下；例如 ZSXQ 源数据刷新入口现已迁到 `.cursor/skills/series-zsxq-adapter/scripts/crawler_zsxq_100.py`：

```bash
uv run python .cursor/skills/series-zsxq-adapter/scripts/crawler_zsxq_100.py --help
```
