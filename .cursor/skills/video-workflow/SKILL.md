...
```bash
# 必填：media-publisher 仓库路径
export MEDIA_PUBLISHER_DIR=/path/to/media-publisher
```

> `workflow.py publish` 支持 `--media-publisher-dir` 参数；若未传参，会按 `MEDIA_PUBLISHER_DIR` → `~/Codes/yyy_monkey/media-publisher` 的顺序解析。
...
### 命令（在仓库根目录执行）
```bash
cd series/book_zsxq_100ke/lessonXXX
uv run manim -qh -r 1080,1920 --fps 60 --disable_caching animate.py LessonXXXVerticalScenes
```
...
uv run manim -qh -r 1080,1920 --fps 60 --disable_caching animate.py LessonXXVerticalScenes
```
...
### 依赖项目
```
media-publisher 仓库目录（推荐通过环境变量 `MEDIA_PUBLISHER_DIR` 指定）
```

### 安装（首次）
```bash
cd "$MEDIA_PUBLISHER_DIR"
uv pip install -e .
...
# 发布到微信视频号
uv run media-publisher --video ... --platform wechat --script ...

# 同时发布两个平台
uv run media-publisher --video ... --platform both --script ...
```
...
```bash
# 日日生金
uv run python .cursor/skills/video-workflow/scripts/workflow.py --series zsxq --media-publisher-dir "$MEDIA_PUBLISHER_DIR" publish 002

# 孙子兵法
uv run python .cursor/skills/video-workflow/scripts/workflow.py --series sunzi --media-publisher-dir "$MEDIA_PUBLISHER_DIR" publish 06
```
